...
module.exports = (async () => {
    test.describe(() => {
        test(`popup - ${basename(__filename)} - 1`, async ({ page, serviceWorker }) => {
            await test.step('override system theme', async () => {
                await page.goto(`chrome-extension://${serviceWorker.url().split('/')[2]}/dist/index.html`)
                await page.bringToFront()
                await (await page.locator(ids.buttons.settings)).click()

                await (await page.locator(ids.switches.themeOverride)).first().setChecked(true)
